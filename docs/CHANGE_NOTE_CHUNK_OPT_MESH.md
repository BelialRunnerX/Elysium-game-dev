...
- **MicroBrick surfaces** are greedily merged per face plane within a refined cell **and across adjacent refined MicroBricks in the same face chunk** (P0-20; see `CHANGE_NOTE_CHUNK_OPT_MESH_CROSSBRICK.md`). Cross-chunk / cube-face (cross-macro) micro merge remains deferred. P0-50 incremental remesh is still whole-brick/chunk. **P0-21 binary/bitmask solid-solid culling is Present** on planar VoxelMesher, spherical LOD0 macro (snapshot + cached), and MicroBrick fill (per-brick columns feeding the stitched greedy mask; see `CHANGE_NOTE_CHUNK_OPT_BITMASK.md`).
...
- Micro 4³ cube inside a refined cell: micro quads strongly reduced (test-locked).
- Isolated 2×2 full-stone MicroBrick pad: micro quads collapse below the within-brick 16-quad bound (test-locked).
...
| File | Change |
|---|---|
| `src/world/PlanetSurfaceMesher.cpp` | face-grid greedy macro + within-brick and cross-brick micro greedy |
...
| `CHANGE_NOTE_CHUNK_OPT_MESH_CROSSBRICK.md` | P0-20 cross-brick follow-up |

## Constraints respected

- Cube-sphere seam correctness: greedy stays within one face chunk; halo neighbor tests unchanged; tiled macro→refined boundaries not greedied across micro. Cross-brick merge does not wrap U/V onto another cube face.
- No EnTT in world.
